...
投影後，每一筆資料會由原本的 784 個變數，
轉換成由所選擇的 $k$ 個主成分所表示的資料

$$
Z=X_cV_k
$$

其中 $Z$ 即為 PCA 最後得到的降維結果
## 投影結果的意義

投影後，每一張影像會在每個主成分方向上得到一個投影分數（Score）。
...
- $x_a^c$：第 $a$ 張影像中心化後的資料
- $v_1$：第一主成分所代表的特徵方向
- $z_{a1}$：第 $a$ 張影像在第一主成分方向上的投影分數
...
因此，每一張影像經過 PCA 後，都可以用不同主成分上的投影分數表示。
...
即可將原本 784 維的資料，以 PC1 與 PC2 兩個方向表示。

![PCA投影結果](PCA_Projection.png)
...
不同數字在 PC1、PC2 方向上的投影範圍並不完全相同

**投影範圍較大：**

代表該類別的樣本在此主成分方向上的分布較分散

**投影範圍較小：**

代表該類別的樣本在此主成分方向上的分布較集中

因此，可以透過觀察不同類別在主成分空間中的投影分布，
了解不同類別在這些主要變異方向上的差異
...
> **找出資料變異最大的方向，並用較少的主成分表示原始資料**

整體流程：

**1. 中心化（Mean Centering）**

移除資料整體位置對分析的影響，使 PCA 專注於資料的變化

**2. 共變異矩陣（Covariance Matrix）**

整理各變數本身的變異，以及變數之間的共同變化關係

**3. 特徵分解（Eigendecomposition）**
...
- **EVR** → 單一主成分所保留的變異比例
- **CEVR** → 前幾個主成分累積保留的變異比例

透過 CEVR 決定需要保留多少個主成分。

**5. 投影（Projection）**

將中心化後的資料投影到選出的主要變異方向：

$$
Z=X_cV_k
$$

最後將原本的 784 維資料轉換成較低的 $k$ 維資料
...
> **簡單來說：PCA 先找出資料變異最大的方向，再選擇重要的方向，最後將資料投影到這些方向上，以較少的維度表示原始資料**
